...
if (isset($_POST) && !empty($_POST)) {
    $sql = "UPDATE Adresse SET Pays = :pays, Adresse = :adresse_part1, Complement = :adresse_part2, Ville = :ville, Code_postal = :code_postal WHERE Id_adresse = :id_adresse && Id_client = :id_client";
...
$sql = "SELECT * FROM Adresse WHERE Id_client = :id_client && Id_adresse = :id_adresse";
$stmt = $conn->prepare($sql);
$stmt->bindValue(":id_client", htmlspecialchars($_SESSION["Id_client"]), PDO::PARAM_INT);
$stmt->bindValue(":id_adresse", htmlspecialchars($parts[2]), PDO::PARAM_INT);
$stmt->execute();
...
<head>
    ...
    <link rel="stylesheet" href="/style/Add_adresse.css">
...
<body>
    ...
    <main>
        <form class="div_ajout" method="post" action='<?php echo $_SERVER["REQUEST_URI"]; ?>'>
            ...
            <select name="pays">
                <option value='<?php echo $data[0]["Pays"] ?>' selected='selected'><?php echo $data[0]["Pays"] ?></option>
            ...
            <h4>Adresse</h4>
            <input type="text" name="adresse_part1" placeholder="Adresse" value="<?php echo $data[0]["Adresse"] ?>">
            <input type="text" name="adresse_part2" placeholder="Apt, suite,unité,nom de l'entreprise (facultatif)" value="<?php echo $data[0]["Complement"] ?>">
            <div class="ville_codePost">
                <div class="code_post">
                    <h4>Code postal</h4>
                    <input name="code_postal" type="number" placeholder="06130" value="<?php echo $data[0]["Code_postal"] ?>">
                </div>
                <div class="ville">
                    <h4>Ville</h3>
                        <input name="ville" type="text" placeholder="Grasse" value="<?php echo $data[0]["Ville"] ?>">
                </div>
            </div>
